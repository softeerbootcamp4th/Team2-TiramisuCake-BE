...
public class DrawDatabaseTest {
    ...
    public DrawModalResponseDto participateDrawEvent(Integer userId) {
        ...
        drawDatabaseUtil.increaseDrawParticipationCount(); // 추첨 이벤트 참여자수 증가
        shareInfoRepository.decreaseRemainDrawCount(userId); // 횟수 1회 차감

        // 만약 당첨 목록에 존재한다면 이미 오늘은 한 번 당첨됐다는 뜻이므로 LoseModal 반환
        int ranking = drawDatabaseUtil.getRankingIfWinner(userId); // 당첨 목록에 존재한다면 랭킹 반환
        if (ranking != 0) {
        ...
        if (drawUtil.isDrawWin()) { // 당첨자일 경우
            ...
            if (drawDatabaseUtil.isWinner(userId, ranking, winnerNum)) { // 레디스에 추첨 티켓이 남았다면, 레디스 당첨 목록에 추가
                // 추첨 티켓이 다 팔리지 않았다면
